#include <muse_mcl_2d_odometry/models/differential_drive.h>

#include <class_loader/register_macro.hpp>
#include <cslibs_math/common/angle.hpp>
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_odometry::DifferentialDrive,
                            muse_mcl_2d::PredictionModel2D)

namespace muse_mcl_2d_odometry {
std::shared_ptr<DifferentialDrive::Result> DifferentialDrive::apply(
  ...
  const double sigma_rot_hat1 =
      std::sqrt(alpha_1_ * sq(delta_rot_noise1) + alpha_2_ * sq(delta_trans));
  const double sigma_trans_hat =
      std::sqrt(alpha_3_ * sq(delta_trans) + alpha_4_ * sq(delta_rot_noise1) +
                alpha_4_ * sq(delta_rot_noise2));
  const double sigma_rot_hat2 =
      std::sqrt(alpha_1_ * sq(delta_rot_noise2) + alpha_2_ * sq(delta_trans));
