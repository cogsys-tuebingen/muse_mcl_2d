...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_odometry::DifferentialDriveBoxMuller,
...
std::shared_ptr<DifferentialDriveBoxMuller::Result>
DifferentialDriveBoxMuller::apply(
    const std::shared_ptr<data_t const> &data,
    const time_t &until, sample_set_t::state_iterator_t states) {
  /// t^O_0 ------------------ t^O_1
  /// ---------- t^s
  /// ----------------- t^u
  ///
  ...
  for (muse_mcl_2d::Sample2D::state_t &sample : states) {
    const double delta_rot_hat1 = cslibs_math::common::angle::difference(
        delta_rot1, boxMuller(sigma_rot_hat1));
    ...
    const double delta_rot_hat2 = cslibs_math::common::angle::difference(
        delta_rot2, boxMuller(sigma_rot_hat2));
    const double tx =
