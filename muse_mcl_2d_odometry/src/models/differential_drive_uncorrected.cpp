...
DifferentialDriveUncorrected::Result::Ptr DifferentialDriveUncorrected::apply(const cslibs_plugins_data::Data::ConstPtr &data,
                                                                              ...
                                                                              sample_set_t::state_iterator_t             states)
{
    ...
    if(delta_trans < eps_zero_linear_ &&
        ...
        return DifferentialDriveUncorrected::Result::Ptr(new Result2D(0.0, 0.0, apply, leave));
    }
    ...
    const double sigma_rot_hat1  = (alpha_1_ * sq(delta_rot_noise1) +
                                    alpha_2_ * sq(delta_trans));
    const double sigma_trans_hat = (alpha_3_ * sq(delta_trans) +
                                    alpha_4_ * sq(delta_rot_noise1) +
                                    alpha_4_ * sq(delta_rot_noise2));
    const double sigma_rot_hat2 =  (alpha_1_ * sq(delta_rot_noise2) +
                                    alpha_2_ * sq(delta_trans));
    ...
    return DifferentialDriveUncorrected::Result::Ptr(new Result2D(delta_trans, std::abs(delta_angular), apply, leave));
}
