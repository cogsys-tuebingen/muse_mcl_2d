#include <muse_mcl_2d_odometry/models/differential_drive_uncorrected.h>

#include <class_loader/register_macro.hpp>
#include <cslibs_math/common/angle.hpp>
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_odometry::DifferentialDriveUncorrected,
                            muse_mcl_2d::PredictionModel2D)

namespace muse_mcl_2d_odometry {
std::shared_ptr<DifferentialDriveUncorrected::Result>
DifferentialDriveUncorrected::apply(const std::shared_ptr<data_t const> &data,
                                    const time_t &until,
  ...
  if (delta_trans < eps_zero_linear_ &&
    ...
    return std::shared_ptr<Result>(
        new Result2D(0.0, 0.0, apply, leave));
  }
  ...
  const double sigma_rot_hat1 =
      (alpha_1_ * sq(delta_rot_noise1) + alpha_2_ * sq(delta_trans));
  const double sigma_trans_hat =
      (alpha_3_ * sq(delta_trans) + alpha_4_ * sq(delta_rot_noise1) +
       alpha_4_ * sq(delta_rot_noise2));
  const double sigma_rot_hat2 =
      (alpha_1_ * sq(delta_rot_noise2) + alpha_2_ * sq(delta_trans));
  ...
  for (muse_mcl_2d::Sample2D::state_t &sample : states) {
    ...
    const double delta_trans_hat =
        (delta_trans - rng_delta_trans_hat_->get()) * sign_trans;
  ...
  return std::shared_ptr<Result>(
      new Result2D(delta_trans, std::abs(delta_angular), apply, leave));
}

void DifferentialDriveUncorrected::doSetup(ros::NodeHandle &nh_private) {
