...
void StatePublisher::setup(ros::NodeHandle &nh) {
  ...
  if (tf_publish) {
    tf_publisher_.reset(new TFPublisher(odom_frame_, base_frame_, world_frame_,
                                        tf_timeout, tf_keep_alive));
    tf_publisher_->start();
  }
}
...
void StatePublisher::publish(const sample_set_t::ConstPtr &sample_set) {
  ...
  SampleDensity2D::ConstPtr density =
      std::dynamic_pointer_cast<SampleDensity2D const>(
          sample_set->getDensity());

  if (!density) {
    std::cerr << "[StatePublisher]: Incompatible sample density estimation!"
              << "\n";
    return;
  }
