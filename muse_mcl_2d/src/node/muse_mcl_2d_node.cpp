...
bool MuseMCL2DNode::requestGlobalInitialization(
    muse_mcl_2d::GlobalInitialization::Request &req,
    muse_mcl_2d::GlobalInitialization::Response &res) {
  auto get_time = []() { return cslibs_time::Time(ros::Time::now().toNSec()); };
...
bool MuseMCL2DNode::requestPoseInitialization(
  ...
  particle_filter_->requestStateInitialization(get_time(), convert_pose(),
                                               convert_covariance());
  res.success = true;
  return true;
}

void MuseMCL2DNode::poseInitialization(
  ...
  particle_filter_->requestStateInitialization(convert_time(), convert_pose(),
                                               convert_covariance());
}
...
bool MuseMCL2DNode::setup() {
  ...
  {  /// Update Models
    loader.load<UpdateModel2D, cslibs_math_ros::tf::TFProvider::Ptr,
                ros::NodeHandle &>(update_models_, tf_provider_backend_,
                                   nh_private_);
  ...
  {  /// Prediction Model
    loader.load<PredictionModel2D, cslibs_math_ros::tf::TFProvider::Ptr,
                ros::NodeHandle &>(prediction_model_, tf_provider_backend_,
                                   nh_private_);
  ...
  {  /// Data Providers
    /// load data plugins
    cslibs_plugins::PluginLoader data_loader("cslibs_plugins_data",
                                             nh_private_);
    data_loader.load<cslibs_plugins_data::DataProvider,
                     cslibs_math_ros::tf::TFProvider::Ptr, ros::NodeHandle &>(
  ...
  {  /// sampling algorithms
    loader.load<UniformSampling2D, map_provider_map_t,
                cslibs_math_ros::tf::TFProvider::Ptr, ros::NodeHandle &>(
    ...
    loader.load<NormalSampling2D, map_provider_map_t,
                cslibs_math_ros::tf::TFProvider::Ptr, ros::NodeHandle &>(
    ...
    loader.load<Resampling2D, UniformSampling2D::Ptr, NormalSampling2D::Ptr,
                ros::NodeHandle &>(resampling_, uniform_sampling_,
                                   normal_sampling_, nh_private_);
    if (!resampling_) {
  ...
  {  /// density estimation
    loader.load<SampleDensity2D, ros::NodeHandle &>(sample_density_,
                                                    nh_private_);
    if (!sample_density_) {
  ...
  {  /// scheduling
    loader.load<Scheduler2D, const update_model_map_t &, ros::NodeHandle &>(
  ...
  //// set up the necessary functions for the particle filter
  {
    const std::string world_frame =
        nh_private_.param<std::string>("world_frame", "/world");

    auto param_name = [](const std::string &param) {
      return "particle_filter/" + param;
    };

    const double resampling_threshold_linear = nh_private_.param<double>(
        param_name("resampling_threshold_linear"), 0.1);
    const double resampling_threshold_angular =
        cslibs_math::common::angle::toRad(nh_private_.param<double>(
            param_name("resampling_threshold_angular"), 5.0));
    const bool resampling_threshold_exceed_both = nh_private_.param<bool>(
        param_name("resampling_threshold_exceed_both"), false);

    const double update_threshold_linear =
        nh_private_.param<double>(param_name("update_threshold_linear"), 0.0);
    const double update_threshold_angular = cslibs_math::common::angle::toRad(
        nh_private_.param<double>(param_name("update_threshold_angular"), 0.0));
    const bool update_threshold_exceed_both = nh_private_.param<bool>(
        param_name("update_threshold_exceed_both"), false);

    const bool use_amcl_diff_calculation =
        nh_private_.param<bool>(param_name("use_amcl_diff_calculation"), false);

    use_amcl_diff_calculation
        ? prediction_integrals_.reset(new prediction_integrals_t(
              std::shared_ptr<PredictionIntegralAMCL2D>(
                  new PredictionIntegralAMCL2D(
                      resampling_threshold_linear, resampling_threshold_angular,
                      resampling_threshold_exceed_both))))
        : prediction_integrals_.reset(new prediction_integrals_t(
              std::shared_ptr<PredictionIntegral2D>(new PredictionIntegral2D(
                  resampling_threshold_linear, resampling_threshold_angular,
                  resampling_threshold_exceed_both))));
    for (const auto &u : update_models_) {
      use_amcl_diff_calculation
          ? prediction_integrals_->set(
                std::shared_ptr<PredictionIntegralAMCL2D>(
                    new PredictionIntegralAMCL2D(update_threshold_linear,
                                                 update_threshold_angular,
                                                 update_threshold_exceed_both)),
                u.second->getId())
          : prediction_integrals_->set(
                std::shared_ptr<PredictionIntegral2D>(new PredictionIntegral2D(
                    update_threshold_linear, update_threshold_angular,
                    update_threshold_exceed_both)),
                u.second->getId());
    }

    const std::size_t sample_size =
        nh_private_.param<int>(param_name("sample_size"), 0);
    const std::size_t minimum_sample_size =
        sample_size == 0
            ? nh_private_.param<int>(param_name("minimum_sample_size"), 0)
            : sample_size;
    const std::size_t maximum_sample_size =
        sample_size == 0
            ? nh_private_.param<int>(param_name("maximum_sample_size"), 0)
            : sample_size;
    const bool enable_lag_correction =
        nh_private_.param<bool>(param_name("enable_lag_correction"), true);
    const bool reset_all_integrals_on_update = nh_private_.param<bool>(
        param_name("reset_all_integrals_on_update"), false);
    const bool reset_all_integrals_on_resampling = nh_private_.param<bool>(
        param_name("reset_all_integrals_on_resampling"), false);
    const bool keep_weights_after_resampling = nh_private_.param<bool>(
        param_name("keep_weights_after_resampling"), false);
    ...
    if (maximum_sample_size < minimum_sample_size) {
      ROS_ERROR_STREAM(
          "Maximum sample size cannot be smaller than minimum sample size!");
      return false;
    }

    while (ros::Time::now().toNSec() == 0) {
    }
    sample_set_.reset(new sample_set_t(
        world_frame, cslibs_time::Time(ros::Time::now().toNSec()),
        minimum_sample_size, maximum_sample_size, sample_density_,
    ...
    particle_filter_->setup(
        sample_set_, uniform_sampling_, normal_sampling_, resampling_,
        state_publisher_, prediction_integrals_, scheduler_,
        reset_all_integrals_on_update, reset_all_integrals_on_resampling,
  ...
  initialization_service_pose_ = nh_private_.advertiseService(
      "/muse_mcl_2d/pose_initialization",
      &MuseMCL2DNode::requestPoseInitialization, this);
  initialization_service_global_ = nh_private_.advertiseService(
      "/muse_mcl_2d/global_initialization",
      &MuseMCL2DNode::requestGlobalInitialization, this);
  initialization_subscriber_pose_ = nh_private_.subscribe(
      "/initialpose", 1, &MuseMCL2DNode::poseInitialization, this);
...
void MuseMCL2DNode::checkPoseInitialization() {
  ...
  if (nh_private_.hasParam("initialization/pose") &&
    ...
    if (p_v.size() != 3) {
      ROS_ERROR_STREAM(
          "The initialization pose is expected to have 3 values [x, y, yaw]");
      return;
    }
    ...
    if (c_v.size() != 9) {
      ROS_ERROR_STREAM(
          "The initialization covariance is expected to have 9 values.");
      return;
    }
...
bool MuseMCL2DNode::getUpdateModelProviderMapping(
    update_model_mapping_t &update_mapping) {
  for (auto &u : update_models_) {
    ...
    const std::string data_provider_param =
        update_model_name + "/data_provider";
    const std::string map_provider_name =
        nh_private_.param<std::string>(map_provider_param, "");
    const std::string data_provider_name =
        nh_private_.param<std::string>(data_provider_param, "");
    if (map_provider_name == "") {
      ROS_ERROR_STREAM("No map provider name can be resolved for update model '"
                       << update_model_name << "'!");
      return false;
    }
    if (data_provider_name == "") {
      ROS_ERROR_STREAM(
          "No data provider name can be resolved for update model '"
          << update_model_name << "'!");
      return false;
    }
    if (map_providers_.find(map_provider_name) == map_providers_.end()) {
      ROS_ERROR_STREAM("Map Provider '"
                       << map_provider_name
                       << "' cannot be found for update model '"
                       << update_model_name << "' !");
      return false;
    }
    if (data_providers_.find(data_provider_name) == data_providers_.end()) {
      ROS_ERROR_STREAM("Data Provider '"
                       << data_provider_name
                       << "' cannot be found for update model '"
                       << update_model_name << "' !");
      return false;
    ...
    cslibs_plugins_data::DataProvider::Ptr data_provider =
        data_providers_.at(data_provider_name);
    update_mapping[update_model] = std::make_pair(data_provider, map_provider);
  }
  return true;
}

bool MuseMCL2DNode::getPredictionProvider(
    cslibs_plugins_data::DataProvider::Ptr &prediction_provider) {
  const std::string param_name =
      prediction_model_->getName() + "/data_provider";
  const std::string provider_name =
      nh_private_.param<std::string>(param_name, "");

  if (data_providers_.find(provider_name) == data_providers_.end()) {
    std::cerr << "[MuseAMCLNode]: Could not find data provider '"
              << provider_name << "' for prediction model '"
              << prediction_model_->getName() << "' !"
              << "\n";
    return false;
  }
