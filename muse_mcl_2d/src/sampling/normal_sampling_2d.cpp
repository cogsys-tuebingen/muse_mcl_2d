...
bool Normal2D::apply(const transform_t& pose, const covariance_t& covariance,
...
void Normal2D::doSetup(
  ...
  for (auto m : map_provider_ids) {
    if (map_providers.find(m) == map_providers.end())
      throw std::runtime_error("[NormalSampling]: Cannot find map provider '" +
                               m + "'!");
...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d::Normal2D,
                            muse_mcl_2d::NormalSampling2D)
