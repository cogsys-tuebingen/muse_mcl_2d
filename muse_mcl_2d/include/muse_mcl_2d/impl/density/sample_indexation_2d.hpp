...
class SampleIndexation2D {
 public:
  ...
  using state_t = Hypothesis2D::state_t;

  inline SampleIndexation2D() { resolution_.fill(0.0); }

  inline SampleIndexation2D(const resolution_t &resolution)
      : resolution_(resolution),
        resolution_inv_{{1.0 / resolution[0], 1.0 / resolution[1]}} {}
  ...
  inline resolution_t const &getResolution() const { return resolution_; }
  ...
  inline index_t create(const Sample2D &sample) const {
    return index_t({
        {static_cast<int>(std::floor(sample.state().tx() * resolution_inv_[0])),
         ...
         static_cast<int>(
             cslibs_math::common::angle::normalize2Pi(sample.state().yaw()) *
             resolution_inv_[1])},
    });
  }
  ...
  inline index_t create(const state_t &state) const {
    return index_t({
        {static_cast<int>(std::floor(state.tx() * resolution_inv_[0])),
         ...
         static_cast<int>(
             cslibs_math::common::angle::normalize2Pi(state.yaw()) *
             resolution_inv_[1])},
    });
  }
  ...
  inline index_t create(const std::array<double, 3> &state) const {
    return index_t(
        {{static_cast<int>(std::floor(state[0] / resolution_[0])),
          ...
          static_cast<int>(cslibs_math::common::angle::normalize2Pi(state[2]) *
                           resolution_inv_[1])}});
  }
