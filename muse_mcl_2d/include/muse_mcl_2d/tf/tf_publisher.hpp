#ifndef MUSE_MCL_2D_TF_PUBLISHER_HPP
#define MUSE_MCL_2D_TF_PUBLISHER_HPP
...
class EIGEN_ALIGN16 TFPublisher {
 public:
  ...
  using queue_t = cslibs_utility::synchronized::queue<
      stamped_t, std::deque<stamped_t, stamped_t::allocator_t>>;
   ...
   * @param world_frame   - the world frame
   */
  inline explicit TFPublisher(const std::string &odom_frame,
 ...
 private:
  ...
  ros::Time tf_last_update_{0};

  inline void loop() {
    auto update_tf = [this](const stamped_t &w_t_b) {
      transform_t b_T_o = transform_t::identity();
      const ros::Time time =
          ros::Time(cslibs_math::utility::tiny_time::seconds(w_t_b.stamp()));
      if (tf_listener_.lookupTransform(base_frame_, odom_frame_, time, b_T_o,
                                       timeout_)) {
        transform_t w_T_o = w_t_b.data() * b_T_o;
        w_T_o_ = tf::StampedTransform(
            cslibs_math_ros::tf::conversion_2d::from(w_T_o), time, world_frame_,
            odom_frame_);
...
#endif  // MUSE_MCL_2D_TF_PUBLISHER_HPP
