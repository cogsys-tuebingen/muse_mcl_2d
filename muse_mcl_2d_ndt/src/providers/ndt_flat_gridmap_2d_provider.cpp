#include "ndt_flat_gridmap_2d_provider.h"

#include <yaml-cpp/yaml.h>

#include <class_loader/register_macro.hpp>
#include <cslibs_ndt_2d/conversion/merge.hpp>
#include <cslibs_ndt_2d/serialization/dynamic_maps/gridmap.hpp>
#include <fstream>
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_ndt::NDTFlatGridmap2DProvider,
                            muse_mcl_2d::MapProvider2D)

namespace muse_mcl_2d_ndt {
NDTFlatGridmap2DProvider::NDTFlatGridmap2DProvider() {}

std::shared_ptr<NDTFlatGridmap2DProvider::state_space_t const>
NDTFlatGridmap2DProvider::getStateSpace() const {
  std::unique_lock<std::mutex> l(map_mutex_);
  if (!map_) map_notify_.wait(l);
  return map_;
}

void NDTFlatGridmap2DProvider::setup(ros::NodeHandle &nh) {
  auto param_name = [this](const std::string &name) {
    return name_ + "/" + name;
  };

  path_ = nh.param<std::string>(param_name("path"), "");
  frame_id_ = nh.param<std::string>(param_name("frame_id"), "/world");

  loadMap();
}

void NDTFlatGridmap2DProvider::loadMap() {
  auto load = [this]() {
    std::unique_lock<std::mutex> l(map_mutex_);
    ROS_INFO_STREAM("Loading file '" << path_ << "'...");
    cslibs_ndt_2d::dynamic_maps::Gridmap<double>::Ptr map;
    if (cslibs_ndt_2d::dynamic_maps::loadBinary<double>(path_, map)) {
      map_.reset(new FlatGridmap2D(
          cslibs_ndt_2d::conversion::merge<double>(map), frame_id_));

      ROS_INFO_STREAM("Successfully loaded file '" << path_ << "'!");
    } else
      throw std::runtime_error("Could not load file '" + path_ + "'!");
    map_notify_.notify_all();
  };

  if (map_) {
    worker_ = std::thread(load);
  } else {
    load();
  }
}
}  // namespace muse_mcl_2d_ndt
