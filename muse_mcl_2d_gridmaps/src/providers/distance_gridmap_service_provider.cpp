...
std::shared_ptr<DistanceGridmapServiceProvider::state_space_t const>
DistanceGridmapServiceProvider::getStateSpace() const {
  if (!map_) {
    nav_msgs::GetMap req;
    if (source_.call(req)) {
      ...
      cslibs_gridmaps::static_maps::conversion::from<double, double>(
          req.response.map, map, binarization_threshold_, maximum_distance_);
      map_.reset(new DistanceGridmap(
