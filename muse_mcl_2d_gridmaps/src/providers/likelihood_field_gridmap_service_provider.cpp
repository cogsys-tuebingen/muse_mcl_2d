...
std::shared_ptr<LikelihoodFieldGridmapServiceProvider::state_space_t const>
LikelihoodFieldGridmapServiceProvider::getStateSpace() const {
  if (!map_) {
    nav_msgs::GetMap req;
    if (source_.call(req)) {
      ...
      cslibs_gridmaps::static_maps::conversion::from<double, double>(
          req.response.map, map, maximum_distance_, sigma_hit_,
          binarization_threshold_);
      map_.reset(new LikelihoodFieldGridmap(
          map, std::string(req.response.map.header.frame_id)));
