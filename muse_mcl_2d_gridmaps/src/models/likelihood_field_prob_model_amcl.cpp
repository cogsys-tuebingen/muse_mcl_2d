...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_gridmaps::LikelihoodFieldProbModelAMCL,
                            muse_mcl_2d::UpdateModel2D)

namespace muse_mcl_2d_gridmaps {
LikelihoodFieldProbModelAMCL::LikelihoodFieldProbModelAMCL() {}

void LikelihoodFieldProbModelAMCL::apply(
  ...
  std::fill(observation_probability_buffer_.begin(),
            observation_probability_buffer_.end(), 0.0);

  {  /// sensor probabilty calculation.
    const auto end = set.const_end();
    std::size_t sample_index = 0;
    for (auto it = set.const_begin(); it != end; ++it, ++sample_index) {
      const state_t m_T_l = m_T_w * it->state() *
                            b_T_l;  /// laser scanner pose in map coordinates
      std::size_t observation_index = 0;
      for (std::size_t i = 0; i < rays_size;
           i += ray_step, ++observation_index) {
        ...
        observation_probability_buffer_[sample_index * max_beams_ +
                                        observation_index] = pz;
      }
    }
  }
  {
    ...
    for (std::size_t i = 0; i < max_beams_; ++i) {
      if (observation_histogram_[i] / static_cast<double>(sample_size) >
    ...
    for (auto it = set.begin(); it != end; ++it, ++sample_index) {
      ...
      for (std::size_t i = 0; i < max_beams_; ++i) {
        if (error || observation_mask_[i]) {
          log_p += log(
              observation_probability_buffer_[sample_index * max_beams_ + i]);
        }
      }
      *it *= std::exp(log_p);
    }
