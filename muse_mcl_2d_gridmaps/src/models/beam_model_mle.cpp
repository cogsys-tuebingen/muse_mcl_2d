...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_gridmaps::BeamModelMLE,
                            muse_mcl_2d::UpdateModel2D)

namespace muse_mcl_2d_gridmaps {
BeamModelMLE::BeamModelMLE() {}

void BeamModelMLE::apply(const std::shared_ptr<data_t const> &data,
  ...
  transform_t b_T_l;
  transform_t m_T_w;
  if (!tf_->lookupTransform(robot_base_frame_, laser_data.frame(),
                            ros::Time(laser_data.timeFrame().end.seconds()),
                            b_T_l, tf_timeout_))
    return;
  if (!tf_->lookupTransform(world_frame_, map->getFrame(),
                            ros::Time(laser_data.timeFrame().end.seconds()),
                            m_T_w, tf_timeout_))
    return;
  ...
  auto p_hit = [this, &pow2](const double ray_range, const double map_range) {
    return parameters_.z_hit * parameters_.denominator_hit *
           std::exp(pow2(ray_range - map_range) *
                    parameters_.denominator_exponent_hit);
  };
  auto p_short = [this](const double ray_range, const double map_range) {
    return ray_range < map_range
               ? parameters_.z_short *
                     (1.0 /
                      (1.0 - std::exp(-parameters_.lambda_short * map_range))) *
                     parameters_.lambda_short *
  ...
  auto p_random = [&range_max, &p_rand](const double ray_range) {
    return ray_range < range_max ? p_rand : 0.0;
  };
  auto probability = [&gridmap, &p_hit, &p_short, &p_max, &p_random,
                      &range_max](const laserscan_t::Ray &ray,
                                  const state_t &m_T_l, double &map_range) {
    const double ray_range = ray.range;
    auto ray_end_point = m_T_l * ray.end_point;
    map_range = std::min(range_max,
                         gridmap.getRange(m_T_l.translation(), ray_end_point));
    return p_hit(ray_range, map_range) + p_short(ray_range, map_range) +
           p_max(ray_range) + p_random(ray_range);
  };
  ...
  for (auto it = set.begin(); it != end; ++it) {
    const state_t m_T_l =
        m_T_w * it.state() * b_T_l;  /// laser scanner pose in map coordinates
    double p = 1.0;
  ...
  if (use_weights_for_estimation_)
    parameter_estimator_mle_->setMeasurements(z, z_bar, particle_weights,
                                              range_max);
  else
...
void BeamModelMLE::doSetup(ros::NodeHandle &nh) {
  ...
  use_estimated_parameters_ =
      nh.param(param_name("use_estimated_parameters"), false);
  use_weights_for_estimation_ =
      nh.param(param_name("use_weights_for_estimation"), false);

  std::size_t max_estimation_iterations = static_cast<std::size_t>(
      nh.param<int>(param_name("max_estimation_iterations"), 20));

  parameter_estimator_mle_.reset(
      new BeamModelParameterEstimator(parameters_, max_estimation_iterations));
}
}  // namespace muse_mcl_2d_gridmaps
