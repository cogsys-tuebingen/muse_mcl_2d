...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_gridmaps::BeamModelLog,
                            muse_mcl_2d::UpdateModel2D)

namespace muse_mcl_2d_gridmaps {
BeamModelLog::BeamModelLog() {}

void BeamModelLog::apply(const std::shared_ptr<data_t const> &data,
  ...
  const std::size_t ray_step =
      std::max(1ul, (rays_size - 1) / (max_beams_ - 1));
  ...
  auto p_hit = [this, &pow2](const double ray_range, const double map_range) {
    return z_hit_ * denominator_hit_ *
           std::exp(pow2(ray_range - map_range) * denominator_exponent_hit_);
  };
  auto p_short = [this](const double ray_range, const double map_range) {
    return ray_range < map_range
               ? z_short_ * lambda_short_ * std::exp(-lambda_short_ * ray_range)
               : 0.0;
    // return ray_range < map_range ? z_short_ * (1.0 / (1.0 -
    // std::exp(-lambda_short_  * map_range))) * lambda_short_ *
  ...
  auto probability = [&gridmap, &p_hit, &p_short, &p_max, &p_random,
    ...
    const double map_range = std::min(
        range_max, gridmap.getRange(m_T_l.translation(), ray_end_point));
