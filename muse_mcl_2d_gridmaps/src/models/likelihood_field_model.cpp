...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_gridmaps::LikelihoodFieldModel,
                            muse_mcl_2d::UpdateModel2D)

namespace muse_mcl_2d_gridmaps {
LikelihoodFieldModel::LikelihoodFieldModel() {}

void LikelihoodFieldModel::apply(
  ...
  for (auto it = set.begin(); it != end; ++it) {
    ...
    for (std::size_t i = 0; i < rays_size; i += ray_step) {
      ...
      const double pz =
          ray.valid() ? p_hit(gridmap.at(ray_end_point)) + p_rand : 1.0;
      p *= pz;
    }
    *it *= p;
  }
