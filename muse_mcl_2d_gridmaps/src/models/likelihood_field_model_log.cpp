...
CLASS_LOADER_REGISTER_CLASS(muse_mcl_2d_gridmaps::LikelihoodFieldModelLog,
                            muse_mcl_2d::UpdateModel2D)

namespace muse_mcl_2d_gridmaps {
LikelihoodFieldModelLog::LikelihoodFieldModelLog() {}

void LikelihoodFieldModelLog::apply(
  ...
  auto p_hit = [this](const double dz) {
    return z_hit_ * denominator_hit_ *
           std::exp(dz * dz * denominator_exponent_hit_);
  };
  ...
  for (auto it = set.const_begin(); it != const_end; ++it, ++it_ps) {
    ...
    for (std::size_t i = 0; i < rays_size; i += ray_step) {
      ...
      const double pz =
          ray.valid() ? p_hit(gridmap.at(ray_end_point)) + p_rand : 0.0;
      p += std::log(pz);
    }
    *it_ps = p;
