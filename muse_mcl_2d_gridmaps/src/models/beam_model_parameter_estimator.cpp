...
BeamModelParameterEstimator::BeamModelParameterEstimator(
    const Parameters &parameters, const std::size_t max_iterations)
    : running_(false),
      stop_(false),
      max_iterations_(max_iterations),
      parameters_(parameters),
      parameters_working_copy_(parameters) {
  ParameterLogger::header_t header = {
      {"z_hit, z_max, z_short, z_rand, sigma_hit, lambda_short"}};
  logger_.reset(new ParameterLogger(header, "/tmp/beam_model.csv"));
  logger_->log(parameters_working_copy_.z_hit, parameters_working_copy_.z_short,
               parameters_working_copy_.z_max, parameters_working_copy_.z_rand,
               parameters_working_copy_.sigma_hit,
               parameters_working_copy_.lambda_short);
}

void BeamModelParameterEstimator::setMeasurements(
    const std::vector<double> &z, const std::vector<double> &z_bar,
...
void BeamModelParameterEstimator::setMeasurements(
    const std::vector<double> &z, const std::vector<double> &z_bar,
    const std::vector<double> &prior, const double range_max) {
  if (running_) return;
...
void BeamModelParameterEstimator::run() {
  ...
  auto p_hit = [this](const double ray_range, const double map_range) {
    ...
    return parameters_working_copy_.z_hit *
           std::exp(-dz * dz *
                    parameters_working_copy_.denominator_exponent_hit);
  };
  auto p_short = [this](const double ray_range, const double map_range) {
    if (ray_range < map_range) {
      return parameters_working_copy_.z_short *
             (1.0 / (1.0 - std::exp(-parameters_working_copy_.lambda_short *
                                    map_range))) *
             parameters_working_copy_.lambda_short *
             exp(-parameters_working_copy_.lambda_short * ray_range);
    }
    return 0.0;
  };
  auto p_max = [this](const double ray_range) {
    if (ray_range >= range_max_) return parameters_working_copy_.z_max * 1.0;
    return 0.0;
  };
  auto p_random = [this, p_rand](const double ray_range) {
    if (ray_range < range_max_) return p_rand;
    return 0.0;
  };
  ...
  while (!terminate(iteration)) {
    ...
    for (std::size_t i = 0; i < measurements_size; ++i) {
      ...
      const double nu =
          1.0 / (p_hit_i + p_short_i + p_max_i + p_rand_i) * prior_[i];
      norm += prior_[i];
      e_hit += nu * p_hit_i;
    ...
    if (parameters_previous.compare(parameters_working_copy_)) break;

    if (!parameters_working_copy_.isNormal()) break;
  ...
  logger_->log(parameters_working_copy_.z_hit, parameters_working_copy_.z_max,
