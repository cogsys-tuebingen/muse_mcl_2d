...
TEST( Test_cslibs_plugins_data, testFindModelNames)
{
    ros::NodeHandle nh{"~"};

    const std::string package_name = "muse_mcl_2d";

    cslibs_plugins::PluginManager<muse_mcl_2d::MapProvider2D> manager(muse_mcl_2d::MapProvider2D::Type(), package_name);
    manager.load();
    ...
    for(const auto &class_name : class_names) {
        ...
        if(constructor) {
            try {
                ...
                std::cerr <<"[cslibs_plugins_data]: " << e.what() << std::endl;
            }
        }
...
TEST( Test_cslibs_plugins_data, testFindProviderNames)
{
    ros::NodeHandle nh{"~"};

    const std::string package_name = "muse_mcl_2d";

    cslibs_plugins::PluginManager<muse_mcl_2d::UpdateModel2D> manager(muse_mcl_2d::UpdateModel2D::Type(), package_name);
    manager.load();
    EXPECT_TRUE(manager.pluginsLoaded());
